...
(function initChatbot() {
  ...
  const zinwearKB = {
    ...
    location: `Nous sommes basés à Abomey-Calavi, Bénin. Contact : contact@zinwear.com | +229 01 43 68 11 18`,
  ...
  async function getAIResponse(userMessage) {
    ...
    // ── Réponses locales prioritaires (rapides, sans API) ──
    ...
    if (msg.includes('fondateur') || msg.includes('mombo') || msg.includes('agbo') || msg.includes('davakan')) {
      return zinwearKB.founders;
    }
    if (msg.includes('produit') || msg.includes('hoodie') || msg.includes('veste') || msg.includes('t-shirt')) {
      return zinwearKB.products;
    }
    if (msg.includes('prix') || msg.includes('combien') || msg.includes('fcfa')) {
      return zinwearKB.price;
    }
    if (msg.includes('livraison') || msg.includes('délai')) {
      return zinwearKB.shipping;
    }
    if (msg.includes('adresse') || msg.includes('calavi') || msg.includes('bénin')) {
      return zinwearKB.location;
    }
    if (msg.includes('merci') || msg.includes('thanks')) {
      return 'Avec plaisir ! Stay ZINWEAR. 🖤';
    }

    // ── Appel API Gemini pour les autres questions ──
    try {
      const API_KEY = 'AIzaSyCz4q1W7gGngBajgno-xSYbYwDGrlqvOfo'; 
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${API_KEY}`,
        {
          ...
          body: JSON.stringify({
            system_instruction: {
              parts: [{
                text: `Tu es l'assistant officiel de ZINWEAR, marque de streetwear premium africaine fondée à Abomey-Calavi, Bénin.
Fondateurs : MOMBO OMIAGA Emmanuel (Directeur Créatif), AGBO Helvis (Directeur Artistique), DAVAKAN Laïs Morgan (Directrice de Collection).
Slogan : "Born in Africa. Worn by the World."
Style : luxe minimaliste, futurisme africain, symboles anciens, palette noir/ivoire/violet.
Produits : hoodies premium (52 000 FCFA), t-shirts oversize (18 000 FCFA), vestes (85 000 FCFA), cargos (42 000 FCFA), accessoires.
Livraison : 3-5 jours au Bénin, 7-14 jours international.
Contact : contact@zinwear.com | +229 01 43 68 11 18
Réponds TOUJOURS en français, avec un ton élégant, premium et bienveillant.
Sois concis (2-3 phrases maximum). Ne parle que de ZINWEAR.`
              }]
            },
            contents: [{
